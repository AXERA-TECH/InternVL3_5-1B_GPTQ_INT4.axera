...
class InternVLGradioDemo:
    ...
    def build_ui(demo: "InternVLGradioDemo", server_name: str = "0.0.0.0", server_port: int = 7860, share: bool = False):
        ...
        with gr.Blocks(title="InternVL3_5-1B_GPTQ_INT4 AX Gradio Demo", theme=gr.themes.Soft(), js=custom_js) as iface:
            gr.HTML("""<style>
            ...
            gr.Markdown("""### InternVL3_5-1B_GPTQ_INT4 图文对话演示\n上传一张图片 (可选)，输入问题，获取中文回答。""")
...
def parse_args():
    parser = argparse.ArgumentParser(description="InternVL3_5-1B_GPTQ_INT4 AX gradio demo")
    parser.add_argument("--hf_model", type=str, default="./InternVL3_5-1B_GPTQ_INT4",
                        help="HuggingFace 模型路径")
    parser.add_argument("--axmodel_path", type=str, default="./InternVL3_5-1B_GPTQ_INT4_axmodel",
                        help="LLM axmodel 目录")
